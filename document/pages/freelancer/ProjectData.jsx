...
const ProjectData = () => {
  ...
  const fetchProject = async(id) =>{
    await axios.get(`http://localhost:5000/fetch-project/${id}`).then(
      (response)=>{
...
// try{
//       await axios.post("http://localhost:5000/make-bid", {clientId, freelancerId,bidAmount,estimatedTime, projectId, proposal}).then(
//         (response)=>{
//             setProposal('');
//             setBidAmount(0);
//             setEstimatedTime(0);
...
// const handleBidSubmit = async () => {
  try {
    const response = await axios.post("http://localhost:5000/makebid", {
      clientId,
      freelancerId,
      bidAmount,
      estimatedTime,
    ...
    console.error("❌ Bidding failed:", err.response?.data || err.message);
    alert("Bidding failed!! Try again!");
  }
};
    ...
    const handleProjectSubmission = async() =>{
      ...
      await axios.post("http://localhost:5000/submit-project", {clientId, freelancerId, projectId, projectLink, manualLink, submissionDescription}).then(
        (response)=>{
            setProjectLink('');
            setManualLink('');
    ...
    const fetchChats = async() =>{
      await axios.get(`http://localhost:5000/fetch-chats/${params['id']}`).then(
        (response) =>{
